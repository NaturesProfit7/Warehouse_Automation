...
from ..core.models import Movement, MovementSourceType, UnmappedItem
from ..core.exceptions import (
    IntegrationError, MappingError, DuplicateMovementError,
    StockCalculationError
)
from ..integrations.keycrm import get_keycrm_client, KeyCRMWebhookPayload
    ...
    async def handle_keycrm_webhook(
        ...
        try:
            ...
            if not validate_keycrm_event(payload):
                logger.info(
                    "Webhook event skipped - not relevant",
                    request_id=request_id,
                ...
                return {
                    "action": "skipped",
                    "reason": "Event not relevant for stock processing",
                    "event": webhook_data.event,
                    "order_id": webhook_data.order_id
                }
...
class WebhookEventLogger:
    ...
    def log_error(self, order_id: Optional[int], error: str, error_type: str):
        """Логирование ошибки."""
        self.error_count += 1
    ...
    def log_skipped(self, reason: str, order_id: Optional[int] = None):
